...
namespace Coffee.UIExtensions
{
...
#endif
	{
		...
		void SetupCommandBuffer()
		{
			...
			if (isOverlay || !Application.isPlaying)
			{
				...
				if (!s_renderedResult)
				{
					s_renderedResult = new Texture2D(screenWidth, screenHeight, TextureFormat.ARGB32, false, false);
					s_renderedResult.filterMode = FilterMode.Point;
				}
				else if (s_renderedResult.width != screenWidth || s_renderedResult.height != screenHeight)
				{
					s_renderedResult.Resize(screenWidth, screenHeight, TextureFormat.ARGB32, false);
				}
				...
				if (!Application.isPlaying)
				{
					...
					s_renderedResult.ReadPixels(new Rect(0, 0, screenWidth, screenHeight), 0, 0);
			...
			// Set properties.
			_buffer.SetGlobalVector(s_EffectFactorId, new Vector4(toneLevel, isOverlay ? 0 : 1));
